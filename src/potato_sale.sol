...
contract POTATOSale is DSAuth, DSExec, DSMath {
    DSToken  public  POTATO;               // The POTATO token itself
    ...
    function initialize(DSToken potato) auth {
        assert(address(POTATO) == address(0));
        assert(potato.owner() == address(this));
        assert(potato.authority() == DSAuthority(0));
        assert(potato.totalSupply() == 0);

        POTATO = potato;
        POTATO.mint(totalSupply);
