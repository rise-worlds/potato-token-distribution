...
import "./potato_sale.sol";

contract TestUser is DSExec {
    TestablePOTATOSale sale;

    function TestUser(TestablePOTATOSale sale_) {
        sale = sale_;
    }
...
contract TestOwner {
    ...
    function TestOwner(TestablePOTATOSale sale_) {
        sale = sale_;
    }
...
contract TestablePOTATOSale is POTATOSale {
    function TestablePOTATOSale(
        uint n, uint128 t, uint o, uint s, uint128 a, string k
    ) POTATOSale(n, t, o, s, a, k) {}
...
contract POTATOSaleTest is DSTest, DSExec {
    TestablePOTATOSale  sale;
    DSToken          POTATO;
    DSGuard          guard;
    TestUser         user1;
    ...
    function setUp() {
        ...
        POTATO = new DSToken("POTATO");
        window = 0;

        sale = new TestablePOTATOSale(
            5, 156.25 ether, now, block.timestamp + 1 days, 10 ether, x
        );

        POTATO.setOwner(sale);
        sale.initialize(POTATO);
    ...
    function testBuyFirstDay() {
        ...
        sale.addTime(1 days);
        sale.claim(0);
        assertEq(POTATO.balanceOf(this), 31.25 ether);
    }
    ...
    function testBuyFirstAndSecondDay() {
        ...
        assertEq(POTATO.balanceOf(this), 54.25 ether);
    }
    ...
    function testAllDistributed() {
        ...
        assertEq(POTATO.balanceOf(this), 146.25 ether);
    }
    ...
    function testMultiUser() {
        ...
        assertEq(POTATO.balanceOf(this), 15.625 ether);
        assertEq(POTATO.balanceOf(user1), 15.625 ether);
    }

    function testMultiUserAsymmetricBid() {
        nextRound(1 ether, 9 ether, 0);
        assertEq(POTATO.balanceOf(this), 3.125 ether);
        assertEq(POTATO.balanceOf(user1), 28.125 ether);
    }

    // is this an issue?
    function testRepeatingDecimalRoundUp() {
        nextRound(1 ether, 1 ether, 1 ether);
        assertEq(POTATO.balanceOf(this), 10416666666666666667);
        assertEq(POTATO.balanceOf(user1), 10416666666666666667);
        assertEq(POTATO.balanceOf(user2), 10416666666666666667);
    }
    ...
    function testRepeatingDecimalRoundDown() {
        ...
        assertEq(POTATO.balanceOf(this), 19166666666666666665);
        assertEq(POTATO.balanceOf(user1), 3833333333333333333);
    }
    ...
    function testMultiUserFinalize() {
        ...
        nextRound(1 ether, 1 ether, 0);
        nextRound(1 ether, 1 ether, 0);
        assertEq(POTATO.balanceOf(this), 73.125 ether);
        assertEq(POTATO.balanceOf(user1), 73.125 ether);
        addTime();
    ...
    function testMultiUserAsymmetricBidFinalize() {
        ...
        assertEq(POTATO.balanceOf(this), 70.675 ether);
        assertEq(POTATO.balanceOf(user1), 41.075 ether);
        assertEq(POTATO.balanceOf(user2), 34.5 ether);
        addTime();
    ...
    function testClaimAfterFinalize() {
        ...
        assertEq(POTATO.balanceOf(this), 0);
    ...
    function testClaimAll() {
        ...
        sale.claimAll();
        assertEq(POTATO.balanceOf(this), 77.25 ether);
    }
...
contract POTATOSalePreInitTests is DSTest {
    TestablePOTATOSale  sale;
    TestUser         user1;
    ...
    function testFailTokenAuthority() {
        DSToken POTATO = new DSToken("POTATO");
        POTATO.setAuthority(new DSGuard());
        POTATO.setOwner(sale);
        sale.initialize(POTATO);
    }
}
